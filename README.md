...
[![npm version](https://img.shields.io/npm/v/agent-sync.svg?logo=npm)](https://www.npmjs.com/package/agent-sync)
...
`agent-sync` inspects your repository (databases, frameworks, language runtimes, conventions) and synthesizes [Agent Skills](https://agentskills.io) tailored to _your_ code — not a generic template. The output is a `.agents/` folder that any modern AI coding agent already understands.
...
| 3. Fetch      | Pulls the base skill template from the agent-sync registry.                      |
...
```bash
agent-sync init            # cache project context, scaffold .agents/
agent-sync add <skill>     # synthesize and inject a skill
agent-sync list            # list available skills in the registry
```
